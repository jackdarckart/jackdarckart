...
(function () {
  ...
  function createStudio() {
    ...
    let supportsWebCodecsMp3 = false;
    let webCodecsMp3Probe = null;
    ...
    function populateFormatOptions(preferredFormatId) {
      if (!elements.formatSelect) {
        return;
      }

      const options = getExportFormats();
      const nextValue = options.some((option) => option.id === preferredFormatId)
        ? preferredFormatId
    ...
    async function renderMaster() {
      ...
      const mp3Support = selectedFormat.id === 'mp3' ? getMp3Support() : null;
      setRenderState('loading', 'Master wird lokal gerendert …');
      elements.renderStatus.textContent = selectedFormat.id === 'mp3' && mp3Support && mp3Support.serverEndpoint && !mp3Support.nativeMimeType && !mp3Support.clientEncoder
    ...
    function getMp3Support() {
      const nativeMimeType = resolveSupportedMimeType(MP3_MIME_TYPES);
    ...
    function refreshMp3Availability() {
      const AudioEncoderCtor = getAudioEncoderConstructor();
      if (typeof AudioEncoderCtor !== 'function' || typeof AudioEncoderCtor.isConfigSupported !== 'function') {
        return Promise.resolve(false);
      }
      if (webCodecsMp3Probe) {
        return webCodecsMp3Probe;
      }
      webCodecsMp3Probe = AudioEncoderCtor.isConfigSupported({
        codec: 'mp3',
        sampleRate: 44100,
        numberOfChannels: 2,
        bitrate: Number(PREFERRED_MP3_BITRATE)
      }).then((result) => {
        supportsWebCodecsMp3 = Boolean(result && result.supported);
    ...
    function getClientMp3Encoder() {
      const explicitEncoder = resolveExplicitMp3Encoder();
      if (explicitEncoder) {
        return explicitEncoder;
      }
      if (supportsWebCodecsMp3) {
        return { id: 'webcodecs' };
      }
    ...
    function resolveExplicitMp3Encoder() {
    ...
    async function encodeMp3Locally(masteredBuffer, bitrate, encoder) {
      ...
      if (encoder.id === 'webcodecs') {
        return encodeMp3WithWebCodecs(masteredBuffer, bitrate);
      }
  ...
  function getAudioEncoderConstructor() {
    return window.AudioEncoder || globalThis.AudioEncoder;
  }

  function getAudioDataConstructor() {
    return window.AudioData || globalThis.AudioData;
  }
  ...
  async function encodeMp3WithWebCodecs(buffer, bitrate) {
    const AudioEncoderCtor = getAudioEncoderConstructor();
    const AudioDataCtor = getAudioDataConstructor();
    if (typeof AudioEncoderCtor !== 'function' || typeof AudioDataCtor !== 'function') {
      throw new Error('WebCodecs-MP3-Encoder ist in diesem Browser nicht verfügbar.');
    }

    const config = {
      codec: 'mp3',
      sampleRate: Math.max(8000, Math.round(buffer.sampleRate) || 44100),
      numberOfChannels: Math.min(2, Math.max(1, buffer.numberOfChannels || 1)),
      bitrate: Number(bitrate) || Number(PREFERRED_MP3_BITRATE)
    };
    if (typeof AudioEncoderCtor.isConfigSupported === 'function') {
      ...
      if (!support || support.supported === false) {
        throw new Error('WebCodecs meldet keinen MP3-Encoder für diese Umgebung.');
      }
    }
    ...
    const encoder = new AudioEncoderCtor({
      output(chunk) {
        if (!chunk || typeof chunk.copyTo !== 'function' || !Number.isFinite(chunk.byteLength)) {
          return;
        }
        const target = new Uint8Array(chunk.byteLength);
        chunk.copyTo(target);
        chunks.push(target);
      },
      error(error) {
        encoderFailure = error || new Error('WebCodecs-MP3-Encoder ist fehlgeschlagen.');
      }
    });
    ...
    for (let offset = 0; offset < buffer.length; offset += frameSize) {
      const frames = Math.min(frameSize, buffer.length - offset);
      const pcm = convertAudioBufferToInterleavedInt16(buffer, offset, frames, config.numberOfChannels);
      const timestamp = Math.round((offset / config.sampleRate) * 1000000);
  ...
  function convertAudioBufferToInterleavedInt16(buffer, offset, frameCount, channelCount) {
    const pcm = new Int16Array(frameCount * channelCount);
    const channels = [];
    for (let channelIndex = 0; channelIndex < channelCount; channelIndex += 1) {
      channels.push(buffer.getChannelData(Math.min(channelIndex, buffer.numberOfChannels - 1)));
    }
    for (let frameIndex = 0; frameIndex < frameCount; frameIndex += 1) {
      for (let channelIndex = 0; channelIndex < channelCount; channelIndex += 1) {
        const source = channels[channelIndex];
        const sample = source ? source[offset + frameIndex] || 0 : 0;
        pcm[frameIndex * channelCount + channelIndex] = floatToInt16Sample(sample);
      }
    }
    return pcm;
  }
